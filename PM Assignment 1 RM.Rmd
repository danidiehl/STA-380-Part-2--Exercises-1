...
ms = ms/rowSums(ms)
# ms[ms == 0] <- NA
X = ms[,c(-1,-36, -37)] #remove user, spam and adult
X = scale(X, center=TRUE, scale=TRUE)

set.seed(1)
ms$cluster <- kmeanspp(X, k=10, nstart=20)$cluster

#See Cluster 7 is Outdoorsy, Personal Fitnessy, Healthy
...
#See Cluster 5, so high cooking, beauty, fashion, photo sharing
...
shopping<- gather(data = ms, -shopping, -adult, -spam, -uncategorized, -cluster, key = "var", value = "value")
...
# #clust2 = kmeans(X, 4, nstart=25)
# clust1$center
# 
# which(clust1$cluster == 1)
# qplot(, religion, data=ms, color=factor(clust1$cluster))
# qplot(health_nutrition, food, data=ms, color=factor(clust1$cluster))
# qplot(parenting, religion, data=ms, color=factor(clust1$cluster))
# 
# kset = c(5,10,15)
# withinss = c()
# foreach (i = 1:length(kset)) %do% {
#     cat('doing cluster: k',kset[i],'nstart', 20,'\n')
#   clust2 = kmeanspp(X, k=kset[i], nstart=20)
#   withinss = append(withinss,clust2$tot.withinss)
# }
# 
# qplot(health_nutrition, sports_fandom, data=ms, color=factor(clust2$cluster))
# qplot(parenting, religion, data=ms, color=factor(clust2$cluster))  
